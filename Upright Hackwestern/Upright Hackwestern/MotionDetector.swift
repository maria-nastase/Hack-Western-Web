...
class MotionDetector: ObservableObject {
    ...
    @Published var zAccelerations: [Double] = [0.0]
    @Published var yAccelerations: [Double] = [0.0]
    @Published var xAccelerations: [Double] = [0.0]
    ...
    func updateMotionData() {
        if let data = motionManager.deviceMotion {
            ...
            zAccelerations.append(data.userAcceleration.z)
            if zAccelerations.count > SEQUENCE_LENGTH {
                zAccelerations.removeFirst()
            }
            yAccelerations.append(data.userAcceleration.y)
            if yAccelerations.count > SEQUENCE_LENGTH {
                yAccelerations.removeFirst()
            }
            xAccelerations.append(data.userAcceleration.x)
            if xAccelerations.count > SEQUENCE_LENGTH {
                xAccelerations.removeFirst()
            }
